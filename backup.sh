...
if [ ! -z "$(echo $MULTI_FILES | grep -i -E "(yes|true|1)")" ]; then
  ...
  for DB in $DATABASES; do
    DB_SIZE=`mysql $MYSQL_HOST_OPTS -e "SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) as 'DB_SIZE' FROM information_schema.tables WHERE table_schema='$DB'" | grep -Ev "(DB_SIZE)"`

    if [ "`echo "${DB_SIZE} < ${DB_SIZE_THRESHOLD}" | bc`" -eq 1 ]; then
      ...
      echo "Skipping dump of ${DB} from ${MYSQL_HOST} because ${DB_SIZE} is greater than threshold of ${DB_SIZE_THRESHOLD}..."
    fi
  done
# Multi file: no
else
  DB_SIZE=`mysql $MYSQL_HOST_OPTS -e "SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) as 'DB_SIZE' FROM information_schema.tables WHERE table_schema='$MYSQLDUMP_DATABASE'" | grep -Ev "(DB_SIZE)"`
